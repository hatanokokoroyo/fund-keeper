...
bollinger_bands_time_window = 20
...
def main():
    ...
    return_test(strategy)


def return_test(strategy):
    """
    计算收益率
    :param strategy: 策略
    ...
    total_years_avg_return_rate = 0
    for fund_code in fund_code_list:
        # 指定时间段内的年化利率
        years_avg_return_rate, fund_name = calculate_years_avg_return_rate(fund_code, strategy)
        if years_avg_return_rate == 0:
            return_rate_is_zero_fund_code_list.append({fund_code, years_avg_return_rate, fund_name})
            continue
        if years_avg_return_rate < 0.05:
            return_rate_less_than_5_fund_code_list.append([fund_code, years_avg_return_rate, fund_name])
        else:
            return_rate_greater_than_5_fund_code_list.append([fund_code, years_avg_return_rate, fund_name])
        total_years_avg_return_rate += years_avg_return_rate
    valid_fund_count = (len(fund_code_list) - len(return_rate_is_zero_fund_code_list))
    if valid_fund_count == 0:
        print('没有有效的基金')
        return
    print('三年平均年化利率: ' + str(total_years_avg_return_rate / valid_fund_count))
...
def calculate_years_avg_return_rate(fund_code, strategy):
    """
    计算单个基金的收益率
    ...
    last_buy_date = None
    # 查询基金在指定时间内的历史净值
    fund_name, net_worth_data_list = get_fund_detail2(fund_code)
    # 从第三十天开始计算
    ...
    if total_cost != 0:
        ...
        return_rate = 0
    years_avg_return_rate = round(return_rate / years, 4)
    print(fund_code, 'current period cost:', total_cost, 'total profit:', round(total_profit, 4), ', return rate:',
          years_avg_return_rate, fund_name)
    # net_worth_data_list生成折线图
    show_chart(net_worth_data_list, fund_code, fund_name, years_avg_return_rate)

    return years_avg_return_rate, fund_name


def get_fund_detail2(fund_code: str):
...
def show_chart(net_worth_data_list: list, fund_code, fund_name, return_rate):
    ...
    plt.plot(data_frame['date'], data_frame['middle_band'], color='orange', label='middle band')
